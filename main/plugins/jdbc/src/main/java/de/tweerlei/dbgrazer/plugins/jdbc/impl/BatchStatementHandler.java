 ...
 * @author Robert Wruck
 */
public class BatchStatementHandler implements CountingRowHandler
	{
	 ...
	 */
	public BatchStatementHandler(PreparedStatement ps, PreparedStatementCreatorFactory factory, ResultRowMapper mapper, int commitSize, DMLProgressMonitor monitor, boolean rollback, boolean ignoreErrors)
		{
		this.ps = ps;
	...
	public void endRows()
		...
		try	{
			...
			if (ignoreErrors)
				...
				throw new BadSqlGrammarException("statement", "[endRows]", e);
			}
		finally
			{
			try	{
	...
	private int sendBatch() throws SQLException
		{
		int ret = 0;
		
		for (int rows : ps.executeBatch())
			{
			if (rows < 0 /*== Statement.EXECUTE_FAILED*/)
				{
				if (monitor != null)
					monitor.getFailedStatements().progress(1);
				}
