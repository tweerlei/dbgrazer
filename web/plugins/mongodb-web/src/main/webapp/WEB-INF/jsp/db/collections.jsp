...
>		<div class="menuitem"><span onclick="return showDbDialog(event, 'save-document', { database: '${database}' }, '<fmt:message key="send"/>');"><fmt:message key="send"/></span></div>
</c:if
>	</div></div>
	</ui:headline1>
	...
	<c:set var="links" value="db/${currentConnection.linkName}/databases.html,db/${currentConnection.linkName}/collections.html?database=${database}"
	...
	/><div id="explorer-left"><ui:multilevel query="${query.name}" levels="${query.subQueries}" params="${params}" links="${links}" items="${tabs}" var="rs" varKey="label" varParams="detailParams" varParamString="detailParamString" name="combo"
		><ui:result-database rs="${rs}" label="${label}" link="db/${currentConnection.linkName}/documents.html?database=${database}&amp;collection=%%" targetElement="${targetElement}"
	/></ui:multilevel></div>
