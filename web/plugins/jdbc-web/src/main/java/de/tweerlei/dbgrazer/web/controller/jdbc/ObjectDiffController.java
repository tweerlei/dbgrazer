...
/**
 * Compare structure of a single DB object
 * 
 * @author Robert Wruck
 */
@Controller
public class ObjectDiffController
	{
	/**
	 ...
	 * @return Model
	 */
	@RequestMapping(value = "/db/*/dbcompare2.html", method = RequestMethod.GET)
	public Map<String, Object> showObjectDialog(
	 ...
	 * @return Model
	 */
	@RequestMapping(value = "/db/*/ajax/dbcompare2.html", method = RequestMethod.POST)
	public Map<String, Object> compareObjects(
			@ModelAttribute("model") FormBackingObject fbo
			)
		{
		if (!connectionSettings.isBrowserEnabled())
		...
		try	{
			if ((fbo.getFile() != null) && !fbo.getFile().isEmpty())
				...
				Exception ex = null;
				try	{
					sch = readSingletonSchema(fbo.getFile(), fbo.getObject());
					}
				catch (IOException e)
					{
					ex = e;
	...
	private Map<String, Object> compareObjectsInternal(
			String catalog,
			String schema,
			String object,
			SQLSchema right,
	...
	private Map<String, Object> compareObjectsInternal(
		...
		{
		final SQLSchema right = readSingletonSchema(conn2, catalog2, schema2, object);
	...
	private SQLSchema readSingletonSchema(String conn, String catalog, String schema, String object)
		{
		final SQLSchema ret = new SQLSchema(catalog, schema);
		ret.addTable(metadataService.getTableInfo(conn, new QualifiedName(catalog, schema, object), ColumnMode.ALL));
		return (ret);
		}
	
	private SQLSchema readSingletonSchema(MultipartFile file, String object) throws IOException
		{
		final DBSchemaParser p = new DBSchemaParser();
		final InputStream is = file.getInputStream();
		try	{
			return (p.parseSchema(is, object));
			}
