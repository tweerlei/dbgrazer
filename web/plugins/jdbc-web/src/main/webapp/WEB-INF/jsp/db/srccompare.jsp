...
%><fmt:message key="ddlCompare" var="pageTitle"/><%@
...
%>
	...
	<form id="h1-form" class="h1-actions" action="db/${currentConnection.linkName}/srccompare.html" method="get" onsubmit="return showFormDialog(event, 'db:choose-schema', this, '<fmt:message key="change"/>');">
		<input type="hidden" name="backTo" value="srccompare"/>
	...
	<div class="tab-page"><div id="zoomable1" class="tab-header">
		<spring:form id="submitform" cssClass="full" action="db/${currentConnection.linkName}/ajax/srccompare.html" method="post" modelAttribute="model" onsubmit="return submitForm(this);">
...
>					</spring:select></dd>
				...
				<dt><spring:label path="type"><fmt:message key="dbObjectType"/></spring:label></dt>
				<dd><spring:input path="type"/></dd>
				<dt><fmt:message key="compare"/></dt>
				<dd><fmt:message key="contents" var="lbl"/><spring:radiobutton path="grant" value="false" label=" ${lbl}"/>
					&nbsp; <fmt:message key="privileges" var="lbl"/><spring:radiobutton path="grant" value="true" label=" ${lbl}"/></dd>
