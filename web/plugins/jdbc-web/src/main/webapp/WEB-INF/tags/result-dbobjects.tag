...
%><%@
	attribute name="emptyText" required="false" type="java.lang.String" rtexprvalue="true" description="The message key for empty values"
%><%@
...
>					<th>&nbsp;</th>
</c:if><c:forEach items="${rs.columns}" var="c"
...
</c:if><c:forEach items="${row.values}" var="v" varStatus="st"
>					<td><c:choose
						><c:when test="${rs.attributes['moreLevels']}"><ui:message text="${v}" key="${emptyText}"/></c:when
