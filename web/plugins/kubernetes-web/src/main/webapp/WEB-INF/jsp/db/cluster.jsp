...
%><fmt:message key="kubernetesBrowser" var="pageTitle"/><%@
...
/>
	...
	<div class="h1-actions">
		...
		<a class="action" title="<fmt:message key="newWindow"/>" href="db/${currentConnection.linkName}/cluster.html" target="_blank"><fmt:message key="newWindowIcon"/></a>
	</div>
	...
	<c:set var="links" value="db/${currentConnection.linkName}/cluster.html"
	...
	/><div id="explorer-left"><ui:multilevel query="${query.name}" levels="${query.subQueries}" params="${params}" links="${links}" items="${tabs}" var="rs" varKey="label" varParams="detailParams" varParamString="detailParamString" name="combo"
		><ui:result-apiobjects rs="${rs}" label="${label}" link="db/${currentConnection.linkName}/namespace.html?namespace=%%" targetElement="${targetElement}"
	/></ui:multilevel></div>
