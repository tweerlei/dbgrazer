 ...
 */
package de.tweerlei.dbgrazer.web.extension.file;
...
@Service
@Order(7)
public class FileExtension extends FrontendExtensionAdapter
	{
	 ...
	 */
	@Autowired
	public FileExtension(FrontendHelperService frontendHelper, ConnectionSettings connectionSettings)
		{
		super("File");
	...
	public List<ExtensionLink> getEditMenuExtensions()
		{
		if (!connectionSettings.getType().getName().equals(FileConstants.LINKTYPE_FILE))
		...
		if (connectionSettings.isBrowserEnabled())
			{
			ret.add(new ExtensionLink("fileBrowser", frontendHelper.buildPath(MessageKeys.PATH_DB, connectionSettings.getLinkName(), "files.html", null), null, null));
			}
