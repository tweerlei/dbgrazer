...
var Sort = (function(){
	...
	sort.date.formats = [
		...
		{ re:/(\d{2,4})-(\d{1,2})-(\d{1,2})( (\d{1,2}):(\d{1,2}):(\d{1,2}))?/ , f:function(x){ return (x[4] ? new Date(sort.date.fixYear(x[1]),+x[2]-1,+x[3],+x[5],+x[6],+x[7]) : new Date(sort.date.fixYear(x[1]),+x[2]-1,+x[3])).getTime(); } }
		// DD.MM.YY[YY] hh:mm:ss
		,{ re:/(\d{1,2})\.(\d{1,2})\.(\d{2,4})( (\d{1,2}):(\d{1,2}):(\d{1,2}))?/ , f:function(x){ return (x[4] ? new Date(sort.date.fixYear(x[3]),+x[2]-1,+x[1],+x[5],+x[6],+x[7]) : new Date(sort.date.fixYear(x[3]),+x[2]-1,+x[1])).getTime(); } }
