...
%><form action="#" method="get" onsubmit="return false;">
	<div><ui:filter id="filter-select-1" target="select-1 select-2 select-3"/><hr/></div>
	<div class="float-left"><ui:headline3 key="views"/>
...
<c:forEach items="${allQueries.views}" var="v"
>		<option value="${v}"<c:if test="${v == value}"> selected="selected"</c:if>>${v}</option>
</c:forEach
>	</select></div>
	<div class="float-left"><ui:headline3 key="queries"/>
...
>	</select></div>
	<div class="float-left"><ui:headline3 key="queryParams"/>
	<select name="value3" id="select-3" size="20" ondblclick="return applySelection(this, '${fn:escapeXml(target)}');">
		<option value=""><fmt:message key="noQuery"/></option>
<c:forEach items="${allParams}" var="p"
>		<option value="*${p}"<c:if test="${v == value}"> selected="selected"</c:if>>${p}</option>
</c:forEach
>	</select></div><hr/>
</form>
