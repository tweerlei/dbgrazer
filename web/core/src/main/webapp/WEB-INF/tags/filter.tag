...
><div class="filter"><label for="${id}"><fmt:message key="filter"/>:</label> <input type="text" name="${id}" id="${id}" oninput="selectFilter.start(500, this.value, 'filtercount-${id}'<c:forTokens items="${target}" delims=", " var="f">, '${f}'</c:forTokens>);"/><span id="filtercount-${id}"></span></div><c:if test="${form}"
></form></c:if
>