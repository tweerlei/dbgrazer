...
%><%@
	attribute name="defaultTargetElement" required="false" type="java.lang.String" rtexprvalue="true" description="The link target element"
%><%@
...
%><%@
	attribute name="level" required="false" type="java.lang.Integer" rtexprvalue="true" description="Nesting level"
%><%@
...
%><%@
	attribute name="varTarget" required="true" type="java.lang.String" rtexprvalue="false" description="Scoped variable for the link target element"
%><%@
...
%><%@
	variable name-from-attribute="varTarget" alias="targetElement" scope="NESTED"
%><%@
...
%><c:choose
	><c:when test="${level == null}"><c:set var="targetElement" value="${defaultTargetElement}"/><div class="tab-page"><div class="tab-body"></c:when
	><c:when test="${level == 0}"><c:set var="targetElement" value="drilldown0"/><div class="tab-page"><div class="tab-body"></c:when
	><c:otherwise><c:set var="targetElement" value="drilldown${level}"/></c:otherwise
></c:choose
		><c:forEach items="${items}" var="r" varStatus="st">
			<c:choose
		...
		></c:forEach>
	<c:choose
	><c:when test="${level == null}"></div></div></c:when
	><c:when test="${level == 0}"><div id="drilldown0"></div></div></div></c:when
	><c:otherwise><div id="drilldown${level}"></div></c:otherwise
></c:choose>