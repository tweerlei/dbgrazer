...
--%><%@
	tag description="Custom tag that generates DL tags from a 2-column RowSet"
%><%@
...
%><c:choose><c:when test="${fn:startsWith(rs.firstRow.values[0], '--')}"
><div class="column${columns}">
		<div class="subheading">${fn:escapeXml(rs.firstRow.values[1])}</div>
		<dl id="result-${label}" class="result">
<c:forEach items="${rs.rows}" begin="1" var="row" varStatus="st"
><c:choose><c:when test="${fn:startsWith(row.values[0], '--')}"
>		</dl><hr/></div>
		<div class="column${columns}">
...
/><c:forEach items="${rs.rows}" var="row" varStatus="st"
